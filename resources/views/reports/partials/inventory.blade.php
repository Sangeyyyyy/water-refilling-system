...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 p-3 h-100">
            <div class="d-flex justify-content-between align-items-center mb-2 text-dark">
                <h6 class="text-muted text-uppercase small fw-bold mb-0">Staff Assets</h6>
    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 p-3 text-dark h-100">
            <div class="d-flex justify-content-between align-items-center mb-2">
...
<div class="row g-4 mb-5">
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="fw-bold text-dark mb-0">Stock Levels vs. Reorder Thresholds</h5>
                <a href="{{ route('inventory.index') }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">Manage</a>
            </div>
            ...
            <div class="mt-4 p-3 bg-light rounded-3 border-start border-primary border-4 mb-3">
                ...
                <p class="small text-muted mb-0">
                    Bars falling below the red line indicate items require urgent procurement.
                </p>
            </div>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-3 py-2 fw-bold text-dark small">{{ $item->item_name }}</td>
                            <td class="small">{{ $item->stock_level }} <small class="text-muted">{{ $item->unit }}</small></td>
                            <td>
                                @if($item->stock_level <= $item->low_stock_threshold)
                                    <span class="badge bg-danger rounded-pill px-2 small">Urgent</span>
                                @elseif($item->stock_level <= $item->low_stock_threshold * 1.5)
                                    <span class="badge bg-warning text-dark rounded-pill px-2 small">Low</span>
                                @else
                                    <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-2 small border-0">High</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
            <h5 class="fw-bold mb-4 text-dark">Circulation Overview</h5>
            ...
            <div class="mt-4 p-3 bg-light rounded-3 border-start border-info border-4">
                ...
                <p class="small text-muted mb-0">Visualizing gallon distribution across sectors.</p>
            </div>
            <div class="mt-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        if (!document.getElementById('stockHealthChart')) return;
